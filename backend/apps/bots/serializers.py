...
class BotSerializer(serializers.ModelSerializer):
    ...
    deliveryMode = serializers.CharField(source='delivery_mode', read_only=True)
    webhookUrl = serializers.CharField(source='webhook_url', read_only=True)
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'created_at', 'updated_at', 'owner', 'delivery_mode', 'webhook_url']
    ...
    def to_representation(self, instance):
        ...
        representation = {
            ...
            'deliveryMode': instance.delivery_mode,
            'webhookUrl': instance.webhook_url or '',
        }
...
class BotCreateSerializer(serializers.ModelSerializer):
    ...
    thinkingBudget = serializers.IntegerField(source='thinking_budget', required=False, allow_null=True)
    telegramToken = serializers.CharField(source='telegram_token', required=False, allow_blank=True)
    deliveryMode = serializers.CharField(source='delivery_mode', required=False, default='polling')
    webhookUrl = serializers.CharField(source='webhook_url', required=False, allow_blank=True, allow_null=True)
    ...
    def validate_telegramToken(self, value):
        ...
        if value and value.strip():
            ...
            if not re.match(r'^\d+:[A-Za-z0-9_-]+$', value):
                raise serializers.ValidationError("Invalid token format. Must be in format: 123456789:ABC...")
        return value

    def validate_deliveryMode(self, value):
        """Validate delivery mode is either 'polling' or 'webhook'."""
        if value not in ['polling', 'webhook']:
            raise serializers.ValidationError("Delivery mode must be either 'polling' or 'webhook'")
        return value
...
class BotUpdateSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        """Use BotSerializer's representation for consistency."""
        return BotSerializer(instance).to_representation(instance)
